...
export default class SlideDeck extends Component{
  constructor(props){
    ...
    this.answerMe = this.answerMe.bind(this);
    this.answerNotMe = this.answerNotMe.bind(this);
  ...
  answerMe(e){
    e.preventDefault();
    this.answerSlide(true);
  }
  answerNotMe(e){
    e.preventDefault();
    this.answerSlide(false);
  }
  answerSlide(value){
      ...
      <div class={style.widgetContainer} ref={(container)=>{ this.container = container; }}>
        ...
        <div class={style.responseContainer}>
          <div class={style.buttons}>
            <a class={style.me} onClick={this.answerMe} href="#">
              {this.props.i18n.t("me")}
            </a>
            <a class={style.notMe} onClick={this.answerNotMe} href="#">
              {this.props.i18n.t("not_me")}
            </a>
          </div>
        </div>
        {this.props.allowBack && this.isBackSlide() && (
          <a class={style.back} onClick={this.handleBack} href="#">
            <img src="https://cdn.traitify.com/assets/images/arrow_left.svg" alt="Back" />
          </a>
        )}
        {this.props.allowFullScreen && (
          <div class={[style.fullScreen, this.props.isFullScreen ? style.fullScreenSmall : ""].join(" ")} onClick={this.handleFullScreen} />
        )}
      </div>
    );
  }
